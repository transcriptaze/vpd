            ...
            static init(moduleOptions) {
                return initPromise || (Module = Object.assign({}, Module, moduleOptions), initPromise = new Promise((resolveInitPromise => {
                    var moduleOverrides = Object.assign({}, Module),
                        ...
                        scriptDirectory = "",
                        read_, readAsync, readBinary, setWindowTitle;
                    ...
                    var ABORT = !1,
                        EXITSTATUS, UTF8Decoder = "undefined" != typeof TextDecoder ? new TextDecoder("utf8") : void 0,
                    ...
                    function createWasm() {
                        ...
                        return wasmBinary || "function" != typeof WebAssembly.instantiateStreaming || isDataURI(wasmBinaryFile) || isFileURI(wasmBinaryFile) || ENVIRONMENT_IS_NODE || "function" != typeof fetch ? _(r) : fetch(wasmBinaryFile, {
                            credentials: "same-origin"
                        }).then((function(t) {
                        ...
                        if ("dylink" == a) {
                            ...
                            for (; t < r;) {
                                ...
                                if (1 === m) l.memorySize = _(), l.memoryAlign = _(), l.tableSize = _(), l.tableAlign = _();
                                    ...
                                    for (u = _(), d = 0; d < u; ++d) c = n(), l.neededDynlibs.push(c);
                                else if (3 === m)
                                    for (var f = _(); f--;) {
                                        var h = n();
                    ...
                    function sigToWasmTypes(e) {
                        for (var t = {
                                ...
                                parameters: [],
                                results: "v" == e[0] ? [] : [t[e[0]]]
                            }, _ = 1; _ < e.length; ++_) r.parameters.push(t[e[_]]), "j" === e[_] && r.parameters.push("i32");
                        return r
                    }
                        ...
                        for (var r in e)
                            if (!isInternalSym(r)) {
                                ...
                                r.startsWith("orig$") && (r = r.split("$")[1], t = !0), GOT[r] || (GOT[r] = new WebAssembly.Global({
                                    value: "i32",
                                    mutable: !0
                                })), (t || 0 == GOT[r].value) && ("function" == typeof _ ? GOT[r].value = addFunction(_) : "number" == typeof _ ? GOT[r].value = _ : err("unhandled export type for `" + r + "`: " + typeof _))
                            }
                    }
                    ...
                    function loadDynamicLibrary(e, t, r) {
                        ...
                        if (_) return t.global && !_.global && (_.global = !0, "loading" !== _.module && mergeLibSymbols(_.module, e)), t.nodelete && _.refcount !== 1 / 0 && (_.refcount = 1 / 0), _.refcount++, r && (LDSO.loadedLibsByHandle[r] = _), !t.loadAsync || Promise.resolve(!0);
                    ...
                    var SYSCALLS = {
                        ...
                        calculateAt: function(e, t, r) {
                            ...
                            var _; - 100 === e ? _ = FS.cwd() : _ = SYSCALLS.getStreamFromFD(e).path;
                            if (0 == t.length) {
                    ...
                    function run(e) {
                        ...
                        e = e || arguments_, runDependencies > 0 || !dylibsLoaded && (preloadDylibs(), dylibsLoaded = !0, runDependencies > 0) || (preRun(), runDependencies > 0 || (Module.setStatus ? (Module.setStatus("Running..."), setTimeout((function() {
                            setTimeout((function() {
                                Module.setStatus("")
                            }), 1), t()
                        }), 1)) : t()))
                    ...
                    class ParserImpl {
                        ...
                        parse(e, t, r) {
                            if ("string" == typeof e) currentParseCallback = (t, r, _) => e.slice(t, _);
                            else {
                    ...
                    class Language {
                        ...
                        query(e) {
                            ...
                            if (!_) {
                                ...
                                let s, a = n.match(QUERY_WORD_REGEX)[0];
                                switch (t) {
                                    case 2:
                            ...
                            for (let e = 0; e < a; e++) {
                                ...
                                for (let t = 0; t < r; t++) {
                                    ...
                                    if (s += SIZE_OF_INT, t === PREDICATE_STEP_TYPE_CAPTURE) n.push({
                                        ...
                                        if ("string" !== n[0].type) throw new Error("Predicates must begin with a literal value");
                                        const t = n[0].value;
                                        let r = !0;
                                        switch (t) {
                                            case "not-eq?":
                                                r = !1;
                                            case "eq?":
                                                if (3 !== n.length) throw new Error("Wrong number of arguments to `#eq?` predicate. Expected 2, got " + (n.length - 1));
                                                if ("capture" !== n[1].type) throw new Error(`First argument of \`#eq?\` predicate must be a capture. Got "${n[1].value}"`);
                                                if ("capture" === n[2].type) {
                                                    const t = n[1].name,
                                                        _ = n[2].name;
                                                    m[e].push((function(e) {
                                                        let n, s;
                                                        for (const r of e) r.name === t && (n = r.node), r.name === _ && (s = r.node);
                                                        return void 0 === n || void 0 === s || n.text === s.text === r
                                                    }))
                                                } else {
                                                    const t = n[1].name,
                                                        _ = n[2].value;
                                                    m[e].push((function(e) {
                                                        for (const n of e)
                                                            if (n.name === t) return n.node.text === _ === r;
                                                        return !0
                                                    }))
                                                }
                                                break;
                                            case "not-match?":
                                                r = !1;
                                            case "match?":
                                                if (3 !== n.length) throw new Error(`Wrong number of arguments to \`#match?\` predicate. Expected 2, got ${n.length-1}.`);
                                                if ("capture" !== n[1].type) throw new Error(`First argument of \`#match?\` predicate must be a capture. Got "${n[1].value}".`);
                                                if ("string" !== n[2].type) throw new Error(`Second argument of \`#match?\` predicate must be a string. Got @${n[2].value}.`);
                                                const _ = n[1].name,
                                            ...
                                            case "set!":
                                                ...
                                                if (n.some((e => "string" !== e.type))) throw new Error('Arguments to `#set!` predicate must be a strings.".');
                                            ...
                                            case "is-not?":
                                                ...
                                                if (n.some((e => "string" !== e.type))) throw new Error(`Arguments to \`#${t}\` predicate must be a strings.".`);
                                                const a = "is?" === t ? u : d;
